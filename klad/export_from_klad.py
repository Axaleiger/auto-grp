...
# -*- coding: utf-8 -*-
"""
Выгрузка данных ЦДП из КЛАД (корпоративное хранилище) → JSON для платформы.

Запуск в корпоративной IDE / Jupyter:
  1. Скопируйте config.example.json → config.json
  2. Пропишите layer_id / field_id и период
  3. python export_from_klad.py --config config.json
...
Режимы:
  ...
  connection.mode = "klad_api"   — вызовы KladApiClient (подставьте реальные эндпоинты)
"""
...
class KladClientBase:
    """Интерфейс. Реализуйте методы под реальные эндпоинты/SQL КЛАД."""
...
class LocalCsvClient(KladClientBase):
    ...
    def fetch_startup(self, layer_id: str) -> dict:
        # Дефолты; в КЛАД подставьте PVT/ГФ/глубины по layer_id
        return {
...
class KladApiClient(KladClientBase):
    """
    Боевой каркас. Замените URL/параметры на реальные запросы КЛАД.
    ...
    def fetch_trunks(self, layer_id: str) -> list:
        # TODO: привести ответ КЛАД к [{w,x,y,x3?,y3?,hz?}]
...
def build_export(cfg: dict, client: KladClientBase) -> dict:
    ...
    export: Dict[str, Any] = {
        ...
        "source": {
            "system": "KLAD" if cfg.get("connection", {}).get("mode") == "klad_api" else "local_csv",
    ...
    for L in cfg.get("layers") or []:
        label = L["label"]
        # В local_csv режиме ключ запросов = label; в klad_api = layer_id
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Выгрузка данных ЦДП из КЛАД → JSON")
    ap.add_argument("--config", default=os.path.join(HERE, "config.json"),
                    help="Путь к config.json (по умолчанию klad/config.json)")
    ...
    if mode == "local_csv":
        ...
        client: KladClientBase = LocalCsvClient(ROOT, folders)
    elif mode == "klad_api":
        klad = (cfg.get("connection") or {}).get("klad") or {}
        token = os.environ.get(klad.get("token_env") or "KLAD_TOKEN", "")
        if not token:
            print("Задайте токен в переменной окружения", klad.get("token_env") or "KLAD_TOKEN")
            return 1
        client = KladApiClient(klad["base_url"], token, int(klad.get("timeout_sec") or 120))
    else:
